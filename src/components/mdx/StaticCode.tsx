...
import { STATIC_HIGHLIGHTS_DARK, STATIC_HIGHLIGHTS_LIGHT } from '@/lib/__generated__/staticHighlights'
...
export function StaticCode({ id }: StaticCodeProps) {
  ...
  const html = theme === 'light' ? STATIC_HIGHLIGHTS_LIGHT[id] : STATIC_HIGHLIGHTS_DARK[id]

  if (!html) return <pre className="p-4 text-sm font-mono">Unknown code block: {id}</pre>
