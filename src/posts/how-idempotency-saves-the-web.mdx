...
Imagine a weekend in March 2022. Students across India order food on Uber Eats. The payment fails. The food arrives anyway. They order again. Same story. Within a few hours the word is out.

<UberReceiptsTeaser />

By the end of the weekend, somewhere around 147 meals had been delivered for free across about 100 cities. The cause? A tiny change at a third-party payment provider — a new status code for retry-charge responses — that Uber Eats interpreted as success.
...
This is exactly the missing piece in March 2022. A payment service provider rolled out a new status type for retry-charge responses. Uber Eats' integration interpreted the new status as a successful payment. Every retry of a failed charge now looked like a fresh success. Food kept getting delivered. Money kept not being charged.

The fix — both then, and in any system that takes retries seriously — is a key-based contract between Uber and the provider, where every payment attempt carries an idempotency key, and the provider's response is anchored to that key. Under that contract, the same retried payment is unambiguously the same payment, regardless of what status code the provider invents next quarter.
