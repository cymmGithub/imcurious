...
When a timer finishes or a click happens, the browser does not slam the callback onto the call stack. That would be chaos — imagine a background worker cutting into the cursor's orbit. Instead, the callback goes into a **waiting area**: the **task queue**.
...
In this example, the fetch callback runs before the setTimeout callback. Even if both resolved at the same instant, the microtask would still execute first — the event loop **always drains the microtask queue before picking up the next task**. The fetch `.then()` enters the microtask queue, and the loop processes it before it even looks at the task queue. Here is the full sequence:

<StepList scenarioId="microtask-priority">
1. `setTimeout` is called — hands its callback to the browser's timer (0ms). It will enter the **task queue**
...
6. It returns — popped off. The initial script task is done. The cursor resumes orbiting. The `setTimeout(0)` callback enters the task queue almost immediately, and when the fetch resolves, its `.then()` enters the microtask queue. Even though the timer fired first, the microtask queue drains before the task queue is checked
</StepList>
