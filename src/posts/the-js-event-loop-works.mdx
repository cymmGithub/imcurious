...
The rendering step is the final piece of the cycle. After it completes (or gets skipped), the event loop starts a brand new cycle — or as it is often called, a new **tick**. Each tick is one full pass: check the task queue, drain microtasks, maybe render. Around and around, forever, until you close the tab.
