...
When you use Promises — `.then()`, `.catch()`, or `async/await` — the callbacks do not go into the regular callback queue. They go into a separate, higher-priority lane called the **microcallback queue**.
...
2. `Promise.resolve().then()` callback enters the **microcallback queue**
3. `console.log("Sync")` runs immediately (synchronous)
4. Global code is done. The event loop checks the microcallback queue **first**
5. "Microtask" prints
6. Microcallback queue is empty. Now the event loop checks the callback queue
7. "Task" prints

<Callout>
Microtasks run whenever the JavaScript stack empties. Not just between tasks — after every task, after every callback, after every event handler. The engine clears the entire microcallback queue before moving on.
</Callout>

And here is the dangerous part: if a microcallback queues another microtask, that one runs too, before any task or rendering gets a chance. Microtasks can **starve everything else**:
...
After the cursor finishes its tasks and clears the microcallback queue, the loop marshal checks one more thing: does the screen need to be repainted?
