...
3. `fetch("/api/starwars")` is called — sends a network request via the browser and returns a Promise immediately — popped off. The request is now in-flight
4. `console.log("Sync")` is pushed onto the stack and runs
5. It returns — popped off. The initial script task is done. The cursor resumes orbiting — both queues are still empty because the fetch is in-flight and the timer is counting down
</StepList>

Notice something subtle: the `fetch()` call itself fires **immediately** when the interpreter hits that line. It does not wait for the rest of your synchronous code to finish. The browser's network thread starts the HTTP request right away, in parallel. What _does_ wait is the `.then()` callbacks — they only run after the fetch resolves _and_ the call stack is empty. So the request is already in-flight while `console.log("Sync")` executes. The `fetch()` function is synchronous; the _result handling_ is asynchronous.

Also notice that the two `.then()` callbacks don't enter the microtask queue at the same time. Each one only gets enqueued when the previous Promise resolves. The event loop drains the entire microtask queue before moving on — even when microtasks create more microtasks.
...
<StepList scenarioId="render-step">
1. `requestAnimationFrame` is called — registers its callback with the browser. It will fire during the next **render step**
2. It returns — popped off
3. `setTimeout` is called — hands its callback to the browser's timer (1000ms). It will enter the **task queue**
4. It returns — popped off
5. `fetch("/api/starwars")` is called — sends a network request via the browser and returns a Promise immediately — popped off. Now all three async operations are in-flight: the rAF callback is waiting for the next paint, the timer is counting down, and the fetch is on the network
</StepList>
...
<Sources sources={[
	{ title: "JavaScript: The Hard Parts, v3", url: "https://frontendmasters.com/courses/javascript-hard-parts-v3/", author: "Will Sentance - Frontend Masters", type: "course" },
	{ title: "Jake Archibald on the web browser event loop, setTimeout, micro tasks, requestAnimationFrame, ...", url: "https://www.youtube.com/watch?v=cCOL7MC4Pl0", author: "Jake Archibald", type: "video" },
	{ title: "Execution model", url: "https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Execution_model", author: "MDN Web Docs", type: "reference" },
	{ title: "Microtask guide", url: "https://developer.mozilla.org/en-US/docs/Web/API/HTML_DOM_API/Microtask_guide", author: "MDN Web Docs", type: "reference" },
]} />

<PostEndNav links={['/the-idempotency-saves-the-web', '/the-browser-paints-a-web-page', '/']} />
