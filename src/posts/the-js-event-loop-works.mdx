...
<Callout>
	The task queue does not run until all global code has finished. You could
	have a million console.log statements after setTimeout(fn, 0), and every
	single one would execute before that task gets its chance. The event loop
...
<Callout>
	A note on terminology: the spec calls them **tasks** and **microtasks**. You
	will see tutorials use "callback queue" or "macrotask queue" — those are
	informal names. The official terms are _task queue_ (`setTimeout`,
	`setInterval`, event handlers) and _microtask queue_ (`Promise.then()`,
